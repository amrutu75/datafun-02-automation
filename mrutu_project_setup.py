...
import pathlib
import sys      
import time  # For time.sleep() in the while loop function
...
def create_prefixed_folders_using_list_comprehension(folder_list: list, prefix: str) -> None:
    ...
    folder_names = [f"{prefix}{name}" for name in folder_list]
...
def create_folders_periodically(duration_seconds: int) -> None:
    ...
    num_folders = 3  # Number of folders to create
    for i in range(1, num_folders + 1):
        folder_name = f"periodic-folder-{i}"
...
def create_standardized_folders(folder_list: list, to_lowercase: bool = False, remove_spaces: bool = False) -> None:
    ...
    for name in folder_list:
        ...
        folder_path = ROOT_DIR / name
        folder_path.mkdir(exist_ok=True)
        logger.info(f"Created folder: {folder_path} (original: {original_name})")
...
def main() -> None:
    ...
    logger.info(f"Byline: {automation_mrutu.get_byline()}")
